...
def GANGenerator(
    ...
    with tf.variable_scope("input_project"):
        output = custom_dense(output, 4 * 4 * dim * 16)
    ...
    if not freeze_early_layers:
        for block_id in range(1, num_blocks + 1):
            with tf.variable_scope(block_name(block_id)):
                output = custom_conv1d_transpose(output, num_filters(block_id), kernel_len, 4, upsample=upsample, he_initialiser_slope=0.0)
                output = batchnorm(output)
            output = sample_norm(tf.nn.leaky_relu(output))
    else:
        ...
        for block_id in range(1, num_blocks):
            with tf.variable_scope(block_name(block_id)):
                output = custom_conv1d_transpose(output, num_filters(block_id), kernel_len, 4, upsample=upsample, trainable=False, he_initialiser_slope=0.0)
                output = batchnorm(output)
            output = sample_norm(tf.nn.leaky_relu(output))

        # Only make the last layer trainable
        with tf.variable_scope(block_name(num_blocks)):
            output = custom_conv1d_transpose(output, num_filters(num_blocks), kernel_len, 4, upsample=upsample, he_initialiser_slope=0.0)
            output = batchnorm(output)
        output = sample_norm(tf.nn.leaky_relu(output))
...
def GANDiscriminator(
    ...
    def from_input(x, block_id):
        return custom_conv1d(x, num_filters(block_id), 1, activation=tf.nn.leaky_relu, he_initialiser_slope=0.0)
    ...
    if not freeze_early_layers:
        for block_id in range(num_blocks, 1, -1):
            with tf.variable_scope(block_name(block_id)):
                ...
                output = custom_conv1d(output, num_filters(block_id - 1), kernel_len, 4, padding="SAME", he_initialiser_slope=0.0)
            output = lrelu(output)
            if block_id > 1:
                output = phaseshuffle(output)

        with tf.variable_scope(block_name(1)):
            output = scalar_concat(output, minibatch_mean_stddev(output))
            output = custom_conv1d(output, num_filters(1 - 1), kernel_len, 4, padding="SAME", he_initialiser_slope=0.0)
        output = lrelu(output)
        ...
        with tf.variable_scope(block_name(num_blocks)):
            ...
            output = custom_conv1d(output, num_filters(num_blocks - 1), kernel_len, 4, padding="SAME", he_initialiser_slope=0.0)
        output = lrelu(output)
        if num_blocks > 1:
            output = phaseshuffle(output)
        ...
        for block_id in range(num_blocks - 1, 1, -1):
            with tf.variable_scope(block_name(block_id)):
                ...
                output = custom_conv1d(output, num_filters(block_id - 1), kernel_len, 4, padding="SAME", trainable=False, he_initialiser_slope=0.0)
            output = lrelu(output)
            if block_id > 1:
                output = phaseshuffle(output)
        ...
        with tf.variable_scope(block_name(1)):
            output = scalar_concat(output, minibatch_mean_stddev(output))
            output = custom_conv1d(output, num_filters(1 - 1), kernel_len, 4, padding="SAME", trainable=False, he_initialiser_slope=0.0)
        output = lrelu(output)
    ...
    with tf.variable_scope("output"):
        output = custom_dense(output, 1)[:, 0]
...
#The functions below are adapted from https://github.com/tensorflow/models/blob/master/research/gan/progressive_gan/layers.py

def minibatch_mean_stddev(input_tensor):
    mean, variance = tf.nn.moments(input_tensor, axes=[0])
    del mean
    return tf.reduce_mean(tf.sqrt(variance))

def scalar_concat(input_tensor, scalar):
    num_dims = input_tensor.shape.ndims
    assert  num_dims > 0, "The input tensor must have 1 dimension at least"

    input_shape = tf.shape(input_tensor)

    return tf.concat([input_tensor, tf.ones([input_shape[i] for i in range(num_dims - 1)] + [1]) * scalar],
                     axis=num_dims - 1)

def he_initialiser_scale(shape, slope=1.0):
    fan_in = np.prod(shape[:-1])

    return np.sqrt(2. / ((1. + slope ** 2) * fan_in))

def custom_layer_implementation(apply_kernel, kernel_shape, bias_shape,
                                activation, he_initialiser_slope, use_weight_scaling, trainable=True):
    kernel_scale = he_initialiser_scale(kernel_shape, he_initialiser_slope)

    initial_scale, post_scale = kernel_scale, 1.0
    if use_weight_scaling:
        initial_scale, post_scale = post_scale, initial_scale

    kernel_initialiser = tf.random_normal_initializer(stddev=initial_scale)

    bias = tf.get_variable("bias", shape=bias_shape, initializer=tf.zeros_initializer(), trainable=trainable)

    output = post_scale * apply_kernel(kernel_shape, kernel_initialiser) + bias
...
def custom_dense(input,
                 units,
                 activation=None,
                 he_initialiser_slope=1.0,
                 use_weight_scaling=True):

    def apply_kernel(kernel_shape, kernel_initialiser):
    ...
    return custom_layer_implementation(apply_kernel,
                                       kernel_shape=(input.shape.as_list()[-1], units),
                                       bias_shape=(units,),
                                       activation=activation,
                                       he_initialiser_slope=he_initialiser_slope,
                                       use_weight_scaling=use_weight_scaling
                                       )

def custom_conv1d_transpose(inputs,
                            filters,
                            kernel_width,
                            stride=4,
                            padding="same",
    ...
    if not isinstance(kernel_width, (list, tuple)):
        kernel_width = [kernel_width]
    kernel_width = list(kernel_width)

    def apply_kernel(kernel_shape, kernel_initialiser):
        return conv1d_transpose(inputs,
                                filters=filters,
    ...
    return custom_layer_implementation(apply_kernel,
                                       kernel_shape=kernel_width + [inputs.shape.as_list()[2], filters],
                                       bias_shape=(filters,),
                                       activation=activation,
                                       he_initialiser_slope=he_initialiser_slope,
                                       use_weight_scaling=use_weight_scaling,
