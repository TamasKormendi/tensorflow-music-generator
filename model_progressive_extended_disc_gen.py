...
so this serves as the attribution.
"""
...
def GANGenerator(
    ...
    if not freeze_early_layers:
        for block_id in range(1, num_blocks + 1):
            with tf.variable_scope(block_name(block_id)):
                with tf.variable_scope("conv1"):
                    output = conv1d_transpose(output, num_filters(block_id), kernel_len, 1, upsample=upsample)
                    output = batchnorm(output)
                    output = tf.nn.leaky_relu(output)
        ...
        for block_id in range(1, num_blocks):
            with tf.variable_scope("conv1"):
                output = conv1d_transpose(output, num_filters(block_id), kernel_len, 1, upsample=upsample, trainable=False)
                output = batchnorm(output)
                output = tf.nn.leaky_relu(output)
            with tf.variable_scope("conv2"):
        ...
        with tf.variable_scope(block_name(num_blocks)):
            with tf.variable_scope("conv1"):
                output = conv1d_transpose(output, num_filters(num_blocks), kernel_len, 1, upsample=upsample)
                output = batchnorm(output)
                output = tf.nn.leaky_relu(output)
            with tf.variable_scope("conv2"):
...
def GANDiscriminator(
    ...
    if not freeze_early_layers:
        for block_id in range(num_blocks, 0, -1):
            with tf.variable_scope(block_name(block_id)):
                with tf.variable_scope("conv1"):
                    output = tf.layers.conv1d(output, num_filters(block_id - 1), kernel_len, 1, padding="SAME")
                    output = lrelu(output)
                with tf.variable_scope("conv2"):
        ...
        with tf.variable_scope(block_name(num_blocks)):
            with tf.variable_scope("conv1"):
                output = tf.layers.conv1d(output, num_filters(num_blocks - 1), kernel_len, 1, padding="SAME")
                output = lrelu(output)
            with tf.variable_scope("conv2"):
                # output = tf.layers.conv1d(output, num_filters(block_id), kernel_len, 4, padding="SAME")
        ...
        for block_id in range(num_blocks - 1, 0, -1):
            with tf.variable_scope(block_name(block_id)):
                with tf.variable_scope("conv1"):
                    output = tf.layers.conv1d(output, num_filters(block_id - 1), kernel_len, 1, padding="SAME", trainable=False)
                    output = lrelu(output)
                with tf.variable_scope("conv2"):
                    # output = tf.layers.conv1d(output, num_filters(block_id), kernel_len, 4, padding="SAME")
