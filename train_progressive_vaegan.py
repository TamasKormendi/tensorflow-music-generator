...
from model_progressive_vaegan import GANGenerator, GANDiscriminator, Encoder, block_name
...
def train(training_data_dir, train_dir, stage_id, num_channels, freeze_early_layers=False, use_mixed_precision_training = False, augmentation_level=0):
    ...
    if use_mixed_precision_training:
        ...
        with tf.variable_scope("E", custom_getter=float32_variable_storage_getter):
            E_mu, E_lv = Encoder(x, G_INIT_INPUT_SIZE, num_blocks=stage_id, freeze_early_layers=freeze_early_layers)
        E_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope = "E")

        E_mu = tf.cast(E_mu, tf.float32)
        E_lv = tf.cast(E_lv, tf.float32)

        G_input = gaussian_sample_layer(E_mu, E_lv)
    ...
    kl_divergence = tf.reduce_mean(
        gaussian_KLD(E_mu, E_lv, tf.zeros_like(E_mu), tf.zeros_like(E_lv))
    )

    reconstruction_loss = - tf.reduce_mean(
        gaussian_log_density(D_real_output, D_fake_output, tf.zeros_like(D_fake_output))
    )

    tf.summary.scalar("Encoder_loss/KL_divergence", tf.reduce_mean(kl_divergence))
    ...
    with tf.variable_scope("optimiser_vars") as var_scope:
        ...
        if use_mixed_precision_training:
            ...
            loss_scale_encoder = 32.0

            E_gradients, E_variables = zip(*E_opt.compute_gradients(E_loss * loss_scale_encoder, var_list=E_vars))
            E_gradients = [gradient / loss_scale_encoder for gradient in E_gradients]
            E_train_op = E_opt.apply_gradients(zip(E_gradients, E_variables))
            ...
            E_train_op = E_opt.minimize(E_loss, var_list=E_vars)
